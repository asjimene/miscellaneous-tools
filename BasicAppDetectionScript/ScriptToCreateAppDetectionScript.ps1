...
$binaryToCheck = "@binaryToCheck" # Path to an executable to check if an application is ARM or Intel x64 (leave $null to skip check)
$binaryToCheckResult = "@binaryToCheckResult" # Expected result for the binaryToCheck set to one of the following (leave at $null to skip check): AMD64 (x64),ARM64,x86 (I386)
...
function Get-PeMachineType {
	...
	try {
		...
		switch ($machine) {
			...
			default {
				return "Unknown or unsupported machine: 0x{0:X4}" -f $machine 
			}
		}
	}
	finally {
...
$addAdditionalCheck = Read-Host "Add additional architecture check (Helpful in environments with ARM devices)? [y/N]"
if ($addAdditionalCheck -eq "y" -or $addAdditionalCheck -eq "Y") {
	$binaryToCheck = Get-FileName -initialDirectory $SelectedApp.InstallLocation
	$binaryToCheckResult = Get-PeMachineType -Path $binaryToCheck
	$DetectionScript = $DetectionScript.Replace("@AppName", $DisplayNameNew).Replace("@AppVersion", $SelectedApp.DisplayVersion).Replace("@WindowsInstaller", $WindowsInstaller).Replace("@SystemComponent", $SystemComponent).Replace("@binaryToCheckResult", $binaryToCheckResult).Replace("@binaryToCheck", $binaryToCheck)
} else {
	$binaryToCheck = '$null'
	$binaryToCheckResult = '$null'
	$DetectionScript = $DetectionScript.Replace("@AppName", $DisplayNameNew).Replace("@AppVersion", $SelectedApp.DisplayVersion).Replace("@WindowsInstaller", $WindowsInstaller).Replace("@SystemComponent", $SystemComponent).Replace("@binaryToCheckResult", $binaryToCheckResult).Replace("@binaryToCheck", $binaryToCheck)
}
